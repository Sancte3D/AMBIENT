...
| SW_PWR power slide switch — type + mechanics | 🟡 **Type + electrical correct** (right-angle, side-actuated SMD slide, SPDT, switches only U_PWR.ON / load-switch EN, µA). Part has **tab mounting pegs** (through-board anchors). **Open:** footprint not yet vendored in repo (`UNVERIFIED — NEEDS HUMAN CHECK`) — must be created with the peg NPTH holes; decoupled enclosure slider is mandatory for robustness (see §2). | r18.69 |

...
## Effects-engine host run — 2026-07-20

- Strict C11 compilation with `-Wall -Wextra -Wpedantic -Wshadow
  -Wconversion -Werror`: passed.
- Effects functional/property verification: **478,857 checks, 0 failures**.
- Existing synthesis/visual verification after linking the new module:
  **2,211,898 checks, 0 failures**.
- Effects AddressSanitizer + UndefinedBehaviorSanitizer run: passed.
- Bit-exact bypass, exact block-partition invariance, arena guards, ping-pong
  timing, long-tail survival, reverse pre-tail alignment, all-mode bounds, and
  randomized control stress: passed.
- Measured effects state: **680 B / 4,096 B**.
- Measured default hot arena requirement: **214,489 B / 245,760 B**, including
  alignment allowance.
- Host DREAM CHAIN benchmark: approximately **95.0× real time** for 512-frame
  blocks. This is a relative regression result, not an STM32 cycle claim.
- Ten deterministic PCM16 WAV demonstrations were rendered: dry, each
  individual effect, full DREAM CHAIN, and true look-ahead reverse reverb.

Target linker placement, DWT cycle capture, cache behavior, codec path, and a
multi-hour no-underrun soak remain mandatory before product release.

## Original package host run — 2026-07-16

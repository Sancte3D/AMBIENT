...
G = [
 ...
 ("Controls", "#0ea5e9", [
   ...
   ("SW1-5","Kailh Choc V1/V2 hotswap socket ×5","cell trigger keys — real keyswitch feel (~3mm travel), still DIGITAL on/off via MCP23017 (switch→I²C expander→MCU). Socket solders to the board, switch clicks in (no solder) (r18.74, ADR-0013 superseded — was HX B3F tactile in r18.73, corrected: that made cells feel identical to the modifier buttons). ⚠ Socket has no clean manufacturer/LCSC part number — keyboard-market item, verify a specific vendor listing before order.","https://chosfox.com/products/kailh-choc-switch-1350-hot-swap-sockets","hand",True),
...
SHEET_INFO = {  # filename -> (title, function); order = reading order
 ...
 "mcp":("I/O & LEDs","MCP23017 + 2× PCA9685 + the 23 LEDs + 10 buttons (5 cells on Kailh Choc hotswap + 5 tactile modifiers, all digital on the expander) (largest sheet)"),
...
<div class="guide">
 ...
 <p class="cells"><b>✅ The 5 cells are digital, but keep a REAL keyswitch feel — read this before placing them (r18.74, ADR-0013 superseded, corrected from r18.73).</b> The cells changed from Gateron-magnetic + Hall-sensor (analog velocity) to a <b>digital on/off switch</b> — but NOT the same small tactile button as the modifiers (that was r18.73, and it made the playable cells feel identical to a plain modifier button, killing the "these are keyboard keys" UX). Fixed in r18.74: cells now use a <b>Kailh Choc V1/V2 hot-swap socket</b> (SMD, solders to the board) — the actual Kailh Choc switch (~3 mm real travel, same class as NuPhy/HE-style keyboards) clicks into it from the front, <b>no soldering of the switch itself</b>, swappable if one dies. Electrically unchanged from r18.73: one pin → <b>MCP23017 GPIO (GPA0–GPA4, nets <code>CELL1..5_BTN</code>)</b>, other pin → GND; MCP internal pull-up (idle=HIGH, pressed=LOW), one shared INT line — still the <b>HiChord Batch 4+ topology (switch → I²C GPIO-expander → MCU)</b>, still <b>no Hall sensor, no RC filter, no STM32 ADC pin used</b> (PC0/PC1/PA4/PB0/PB1 stay freed). <b>⚠ The hot-swap socket itself has no clean manufacturer/LCSC part number</b> — it's a keyboard-market item (same sourcing category the original Gateron switches were in); confirm a specific vendor listing before order. Caps are 3D-printed and clip onto the switch's stem (Choc V1 and V2 use different stem shapes — pick one and match the cap to it). Trade-off unchanged: you lose analog press-depth/velocity — fine for triggering; Hall would only be needed for genuine expressive depth (see ADR-0013).</p>
 <p><b>Assembly — who solders what.</b> JLC reflow-assembles every <b>SMD</b> part (the "JLC" rows in the BOM). The <b>hand-place</b> rows (THT headers + modifier buttons, encoders, the 5 cell hot-swap sockets) and the <b>off-board</b> rows (display module, speakers, battery, knobs, and now the 5 Kailh Choc switches themselves) are fitted <b>by hand, after</b> JLC — by whoever does final assembly. So the board comes back from JLC fully SMD-populated; the modules + mechanical parts + cell switches are then hand-fitted.</p>
 ...
 <p class="disp"><b>One schematic drop-in to add in the GUI (the only thing not in the generated sheets) — the power-off block (ADR-0016, ~10 min).</b> Add 4 parts: <b>U_PWR</b> TPS22918 (C131941) on the LDO input — VIN←<code>+5V_RAIL</code>, VOUT→<code>+5V_SW</code>, ON←<code>PWR_ON</code>; <b>SW_PWR</b> slide switch (C49023766) common→<code>PWR_ON</code>, one throw→<code>+5V_RAIL</code>; <b>R_PWR_PD</b> 100 kΩ (C25803) <code>PWR_ON</code>→GND; <b>C_PWR_SW</b> 10 µF (C15850) <code>+5V_SW</code>→GND; then reroute the LDO (U5) VIN from <code>+5V_RAIL</code> to <code>+5V_SW</code>. Result: the side switch is a real off, while the charger stays ahead of it (still charges when off). Done in the GUI so you can ERC it.</p>
...
<tr><td class="ref">Cells ×5 (digital, real keyswitch)</td><td class="part">MCP23017 GPA0–GPA4</td><td class="fn">SW1–SW5 Kailh Choc hotswap → <code>CELL1..5_BTN</code> (other pin GND)</td><td class="fn"><b>MCP internal pull-up; INT shared</b> (PC0/PC1/PA4/PB0/PB1 now free)</td></tr>
...
<div class="guide">
 ...
 <ul>
  <li>✅ <b>In the repo</b> (<code>kicad/libraries/field_ambience.pretty/</code>, 8 custom): crystal HC-49, boost VQFN-HR, Sunlord inductor, PJ-320D 3.5 mm jack, TS-1088 service button, <b>MST-12D18 power slide switch</b>, TC-1212/HX 12×12 button (THT, modifiers only), + 3D STEPs.</li>
  <li>✅ <b>Also vendored</b> (<code>kicad/libraries/keyswitch-kicad-library/</code>, MIT-licensed, already in <code>fp-lib-table</code>): <b>Kailh Choc hot-swap socket footprints</b> (<code>Switch_Keyboard_Hotswap_Kailh.pretty</code>) — cells use <code>SW_Hotswap_Kailh_Choc_V1V2_Plated_1.00u</code>. Community-verified, widely used in shipped mechanical-keyboard PCBs, but <b>not independently re-verified by this project against Kailh's own mechanical drawing</b> — cross-check pad pitch + mounting-hole positions before order.</li>
  ...
  <li>⚠ <b>Cell hot-swap socket (5×, SW1–5):</b> no clean manufacturer/LCSC part number found — keyboard-market item (e.g. Chosfox "Kailh Choc PG1350 Hot Swap Socket", ~$1.45/10pcs). Confirm a real vendor listing + cross-check the vendored footprint against it before order.</li>
 </ul>

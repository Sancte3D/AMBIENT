...
G = [
 ...
 ("Power", "#ef4444", [
   ...
   ("SW_PWR","ALPS SSSS811101 slide switch","on/off — r18.85 premium swap (TE-grade brand, ~$0.15): ALPS datasheet 'Actuator directions: Horizontal' — natively side-actuated, knob slides parallel to the board, NO motion redirection. Body only 1.4 mm tall → stem window z ≈ 0–1.4 mm above the PCB (much lower than the old MST's 2.3–3.8 mm): side-wall slot + slider cap must reach near board level; cap takes the force, same-axis sliding. Common IS labeled in the ALPS circuit diagram (C = terminal 2). Footprint verified against the ALPS land pattern (3 signal pads 3.0/1.5 mm + 4 ground-frame pads + 2 locator holes ø0.9). Fallback: MST-12D18G3 (C49023766) stays vendored","C109335","jlc",True),
...
<div class="guide">
 ...
 <p class="cells"><b>✅ The 5 cells are digital, but keep a REAL keyswitch feel — read this before placing them (r18.75, ADR-0013 superseded).</b> The cells changed from Gateron-magnetic + Hall-sensor (analog velocity) to a <b>digital on/off switch</b> — but NOT the same small tactile button as the modifiers (that was r18.73, and it made the playable cells feel identical to a plain modifier button, killing the "these are keyboard keys" UX). r18.74 first tried a Kailh Choc hot-swap socket, but that socket had no clean manufacturer part number and needed fiddly small-SMD hand-soldering technique that isn't obvious if you haven't built a hobbyist keyboard before. <b>Fixed in r18.75:</b> the Kailh Choc V1 switch (CPG135001D01) is now <b>directly soldered to the board</b> — 2 THT legs through 2 holes, soldered from the back, exactly the same technique as every other button/connector in this design. No hot-swap socket, no special technique, but the switch is now permanent (not swappable without desoldering). Footprint + 3D STEP pulled straight from LCSC/EasyEDA for this exact part (<code>field_ambience:SW_KailhChoc_CPG1350_THT_2P</code>) — the same method this repo already uses for its other custom footprints (TS-1088, MST-12D18, PJ-320D), not a third-party keyboard library. Electrically unchanged since r18.73: one pin → <b>MCP23017 GPIO (GPA0–GPA4, nets <code>CELL1..5_BTN</code>)</b>, other pin → GND; MCP internal pull-up (idle=HIGH, pressed=LOW), one shared INT line — still the <b>HiChord Batch 4+ topology (switch → I²C GPIO-expander → MCU)</b>, still <b>no Hall sensor, no RC filter, no STM32 ADC pin used</b> (PC0/PC1/PA4/PB0/PB1 stay freed). Caps are 3D-printed and clip onto the switch's stem. Trade-off unchanged: you lose analog press-depth/velocity — fine for triggering; Hall would only be needed for genuine expressive depth (see ADR-0013).</p>
 <p><b>Assembly — who solders what.</b> JLC reflow-assembles every <b>SMD</b> part (the "JLC" rows in the BOM). The <b>hand-place</b> rows (THT headers + modifier buttons + the 5 cell keyswitches, encoders) and the <b>off-board</b> rows (display module, speakers, battery, knobs) are fitted <b>by hand, after</b> JLC — by whoever does final assembly. So the board comes back from JLC fully SMD-populated; the THT parts + modules + mechanical parts are then hand-fitted/soldered.</p>
 ...
 <p class="disp"><b>Power-off block (ADR-0016) — already wired in the generated sheets since r18.81 (nothing to add).</b> In <code>power_tree.kicad_sch</code>: <b>U_PWR</b> TPS22918 (C131941) gates the LDO input — VIN←<code>+5V_RAIL</code>, VOUT→<code>+5V_SW</code>→LDO(U5) VIN, ON←<code>PWR_ON</code> (QOD→VOUT active discharge, CT floats — per TI SLVSD76C); <b>SW_PWR</b> slide switch (r18.85: <b>ALPS SSSS811101</b>, C109335) common(2)→<code>PWR_ON</code>, throw(1)→<code>+5V_RAIL</code>; <b>R_PWR_PD</b> 100 kΩ <code>PWR_ON</code>→GND (default OFF); <b>C_PWR_SW</b> 10 µF on <code>+5V_SW</code>. Result: the side switch is a real off, while the charger sits ahead of it (still charges when off). Your job: ERC it in the GUI + buzz the switch terminals on the physical part before fab (fail-safe if wrong).</p>
 <p><b>Hard blockers before order:</b> ERC pass · layout + routing · PCB outline + mechanical coords (TBD) · USB impedance check · the power-off block above.</p>
</div></section>
...
<tr><td class="ref">Cells ×5 (digital, real keyswitch)</td><td class="part">MCP23017 GPA0–GPA4</td><td class="fn">SW1–SW5 Kailh Choc (direct-solder THT) → <code>CELL1..5_BTN</code> (other pin GND)</td><td class="fn"><b>MCP internal pull-up; INT shared</b> (PC0/PC1/PA4/PB0/PB1 now free)</td></tr>
...
<div class="guide">
 ...
 <ul>
  <li>✅ <b>In the repo</b> (<code>kicad/libraries/field_ambience.pretty/</code>, 9 custom): crystal HC-49, boost VQFN-HR, Sunlord inductor, PJ-320D 3.5 mm jack, TS-1088 service button, <b>ALPS SSSS811101 power slide switch (r18.85, datasheet-verified; MST-12D18 stays as fallback)</b>, TC-1212/HX 12×12 button (THT, modifiers only), + 3D STEPs.</li>
  <li>✅ <b>In the repo, cells (r18.75)</b>: <code>SW_KailhChoc_CPG1350_THT_2P</code> — pulled straight from LCSC/EasyEDA for C400229 via <code>easyeda2kicad</code> (this repo's own documented method, same as TS-1088/MST-12D18/PJ-320D), includes a real 3D STEP. Direct-solder THT, 2 electrical pins + 3 unplated mechanical locator holes.</li>
  ...
  <li>⚠ <b>Modifier buttons (5× HX B3F, C36498965, THT):</b> verify the THT footprint <code>SW_TC1212-7.3_THT_4P</code> fits the 12×12 4-pin THT body of the HX B3F; if the pinout differs, generate a fresh one (<code>easyeda2kicad --full --lcsc_id=C36498965</code>).</li>
  <li>ℹ <b>Cell keyswitch sourcing (5×, SW1–5, C400229):</b> LCSC shows 0 stock at time of writing (restocks are common) and JLC's own page notes their automated PCBA line needs a custom fixture for this THT part — neither blocks hand-soldering it yourself (same technique as the other THT parts here). We dropped the earlier r18.74 hot-swap-socket plan specifically because that socket had <i>no</i> clean part number at all; this switch does.</li>
 </ul>

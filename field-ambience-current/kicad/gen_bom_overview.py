...
G = [
 ...
 ("Controls", "#0ea5e9", [
   ...
   ("Cells","Choc-compatible mech switch ×5","playable keys — DIGITAL (r18.73, HiChord B4+): low-profile mechanical switch → MCP23017 GPIO → I²C (ON/OFF, no velocity). Was Gateron magnetic+Hall.","https://www.kailhswitch.com/mechanical-keyboard-switches/low-profile-switches.html","off",True),
...
<div class="guide">
 ...
 <p class="cells"><b>⚠ The 5 cells are now DIGITAL (changed r18.73) — read this.</b> They are plain <b>low-profile mechanical switches</b> (Choc-compatible, plate-mount), read digitally — the HiChord-Batch-4+ model, not the old magnetic+Hall path. Each switch: one terminal → an <b>MCP23017 (U2) GPIO input</b> (internal pull-up), other terminal → GND; the MCU sees them over <b>I²C only</b> (no analog, no ADC, no calibration). <b>Each cell DOES need a Choc switch footprint on the PCB</b> (from the repo <code>kicad/libraries/keyswitch-kicad-library/</code>) — soldered pads or a Kailh Choc hotswap socket — on the 19 mm MX grid, wired to spare MCP23017 GPIOs. <b>ON/OFF only — no velocity.</b> This <b>removed the 5 Hall sensors + 10 RC parts and freed ADC pins PC0/PC1/PA4/PB0/PB1</b>. ⚠ Generator schematic + firmware (<code>cells.c</code>) still being updated to match.</p>
...
<tr><td class="ref">Cells ×5 (digital)</td><td class="part">MCP23017 GPIO (I²C)</td><td class="fn">low-profile switch → expander input</td><td class="fn"><b>internal pull-up, ON/OFF</b> (r18.73; frees PC0/PC1/PA4/PB0/PB1)</td></tr>

...
Current descriptive working labels and their legacy equivalents: SOUND_NAMES.md.
...
|---|---|---|
| Ensemble | Primary candidate: warm sustained ensemble | Detuned saws, low-pass and stereo chorus. Must justify its distinction from Ambient Pad/PADsynth; test mono and avoid doubling chorus in shared FX. Not yet listening-approved. |
| Morph | Primary candidate: slowly evolving harmonic tone | Phase-related waveform morph with slow movement, distinct from ensemble width. Test useful morph range and upper-register harmonics after the cancellation fix. |
| Pluck | Supporting candidate: sparse woody events | Coupled LPG brightness/amplitude decay gives a distinct transient role. Judge against String/Guembri, including whether its independent mono mode is necessary. |
| Keys | Supporting candidate: restrained tonal accents | FM Index/Body interaction corrected. Keep only if useful at ordinary settings without excessive filtering/reverb; Ratio changes and aliasing remain unresolved. |
| Resonant | Rework candidate: expressive dark reed/bass | Current design emphasizes resonant filter attacks, accent and drive. Subtractive synthesis is useful for ambient, but this particular default/identity must earn its role rather than survive as an imitation. Compare against Ember/Bass/Horn before deciding. |
| Pulse | Redesign candidate; keep/drop listening decision open | The original aggressive hoover/stab has been reworked into a softer sustained PWM texture without attack pitch bend. STORM_ROLE_REVIEW.md and CORE_ROLE_COMPARISON.md document it. Distinction from Ensemble/Morph must still justify its place. |
...
| Source-dependent | Chorus / detune | Ensemble when the source benefits. Ensemble already has chorus: check combined phase cancellation and pitch spread. |
...
Next: Resonant default filter/envelope and register response, then Pluck's
uneven attacks (SUPPORT_LEVEL_CALIBRATION.md). FM Ratio/aliasing and Ambient
held-note tuning remain open. Pulse's keep/drop decision still needs listening;
...
## Pulse implementation checkpoint
...
Ensemble/Morph; this implementation alone does not justify keeping it in the palette.

## Three-core comparison checkpoint

CORE_ROLE_COMPARISON.md documents matched short examples and separate dry
measurements. Pulse is technically distinguishable, but final listening
approval is open. The measured native 11.3 LU Morph/Ensemble phrase gap led to

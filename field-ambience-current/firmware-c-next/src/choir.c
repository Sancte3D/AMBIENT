...
static void start_note(int source, float freq_hz, float amp){
    ...
    for(int k=0;k<NPART;++k){
        ...
        if (V[i].stage==V_IDLE){ v->phA[k]=0.13f*(float)k; v->phB[k]=0.61f*(float)k; }
    }

...
static const char * const VOICE_NAMES[4] = {
    "Pad", "String", "Ember", "Bowed"    /* r19.51: Glass removed (harsh FM) */
};
...
static void load_world_preset(void) {
    ...
    voice_i = w->voice;       if (voice_i > 3) voice_i = 0;   /* r19.47 */
...
void menu_init(const menu_callbacks_t *cbs) {
    ...
    world_i = 0;
    {
        ...
        voice_i = w->voice; if (voice_i > 3) voice_i = 0;   /* r19.47 boot voice */
        space  = w->space_pct;
        shim   = w->shimmer_pct;
        atmos  = w->atmos_pct;
        motion = w->motion_pct;

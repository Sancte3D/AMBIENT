...
static void side_setup(pad_side_t *s, float base_freq, int sign, float voice_pan,
     ...
     * disables this — "fenvAmount: 0.0"). The 0.5 here belongs to the
     * chord-spawn / drone path; using it on cell taps put a continuous
     * filter-open sweep over every note and made the pad sound matschig. */
    s->fenvAmount  = 0.0f;
...
void pad_note_on(uint8_t source, float freq_hz, float amp) {
     ...
     * → Cell 1 left (−0.45), Cell 5 right (+0.15). For source IDs ≥ 5 keep
     * centred (chord-spawn / generative use those). */

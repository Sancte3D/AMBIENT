...
| HIGH | H1: Pico-internes SMPS + interne PLL nicht extern messbar (Audio-Clock-Jitter bei stagniertem Pico-Boot unklar) | **HSE 8 MHz Crystal + 2× 20 pF Load-Caps** als Audio-Sync-Quelle (jitter-arm; H743-internes SMPS für VCAP/VCORE) |
...
| **Y1** | **HSE Crystal 8 MHz (ABRACON ABM3-8.000MHZ-D2Y-T)** ⚠️ **BLOCKED (siehe r18.2)** | **5032 2-SMD 5.0×3.2mm** | **C144380** | **Extended (ABRACON, in stock, ~$0.27)** | **NEU v0.7: Audio-Master-Clock. CL=18 pF (Standard), ±20 ppm Tolerance, ±30 ppm Stability, Op-Temp -40°C bis +85°C, **ESR max 500 Ω** (laut ABRACON-Datasheet Rev 12.03.09 Table 1 für 8 MHz Fundamental — Korrektur r18.2: war fälschlich 140 Ω in r18.1). PLL-Konfig: PLL_M=4 → 2 MHz PFD → PLL_N=480 → 960 MHz VCO → /PLL_P=2 → SYSCLK 480 MHz. **🟠 FINDING F-4:** ESR 500 Ω liegt innerhalb STM32H743 theoretischem Limit (~948 Ω aus Gm_critmax 1.5 mA/V) aber **2.6× über dem Best-Practice 5×-Sicherheits-Margin** (~190 Ω). Crystal-Wechsel auf Low-ESR-Variante empfohlen (z.B. ABM8G-8.000MHZ, ECS-80-18-30B, NX5032GA — alle noch zu sourcen + verifizieren). Volle Analyse: `docs/component_reviews/Y1_ABM3-8.000MHZ-D2Y-T.md`** |
